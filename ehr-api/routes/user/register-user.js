    ...
    (err, user) => {
      if (user) {
        ...
        var options = {
          method: "GET",
          url:
            "http://2factor.in/API/V1/2ab4e5d4-685c-11ea-9fa5-0200cd936042/SMS/" +
            user.phoneNumber +
            "/AUTOGEN",
          headers: {
...
router.post("/verify-otp", (req, res) => {
  ...
  let options = {
    method: "GET",
    url:
      "http://2factor.in/API/V1/2ab4e5d4-685c-11ea-9fa5-0200cd936042/SMS/VERIFY/" +
      sessNum +
      "/" +
      otp,
    headers: {},
...
router.post("/complete-form", async (req, res) => {
  ...
  User.register(
    new User({
      _id: keccak256(details.aadhaarNo).toString("hex"),
      name: details.name,
